...
func (db *loggerRepoPsql) CreateLog(log *Log) error {
	str := `insert into logs(
		...
		date_stop, milliseconds, ip, service, req_id, account_info, success) 
	...
	_, err := db.db.Exec(str,
		...
		log.Service, log.RequestId, log.AccountInfo, log.Success)
	if err != nil {
...
func (db *loggerRepoPsql) CreateArrayLog(logs []Log) error {
	...
	for _, value := range logs {
		...
		values = append(values,
			...
			value.Service, value.RequestId, value.AccountInfo, value.Success)
	}
	...
	str := `insert into logs(
		...
		req_id, account_info, success) values`
